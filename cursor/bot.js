...
// Initialize WhatsApp Client with LocalAuth to persist sessions
// Compute a safer executable path and arguments across platforms
function guessLinuxChromePath() {
    try {
        const candidates = [
...
const DEFAULT_EXEC_PATH = process.platform === 'darwin'
    ? '/Applications/Google Chrome.app/Contents/MacOS/Google Chrome'
    : (process.platform === 'linux' ? guessLinuxChromePath() : undefined);
const EXEC_PATH = process.env.PUPPETEER_EXECUTABLE_PATH || process.env.PUPETEER_EXECUTABLE_PATH || DEFAULT_EXEC_PATH;
if (!EXEC_PATH) {
    console.warn('[bot] No Chrome/Chromium executable found. Set PUPPETEER_EXECUTABLE_PATH to your browser path.');
}
const BASE_ARGS = [
    '--no-sandbox',
    ...
    '--no-first-run',
    '--no-zygote',
    // NOTE: '--single-process' can crash on macOS; omit by default
    '--disable-gpu',
...
const client = new Client({
    authStrategy: new LocalAuth({ 
        ...
        dataPath: getWritableSessionDir(),
        // Enable session data backup
        backupSyncIntervalMs: 300000, // 5 minutes
    }),
    // Disable automatic reconnection to handle it manually
    restartOnAuthFail: false,
    // Increase timeout for QR code scanning
    qrTimeoutMs: 5 * 60 * 1000, // 5 minutes
    // Disable takeover on process exit
    takeoverOnConflict: false,
    takeoverTimeoutMs: 0,
    puppeteer: {
        headless: computeHeadless(),
...
client.on('disconnected', (reason) => {
    ...
    // Only attempt to reconnect if it wasn't a manual logout
    if (reason !== 'LOGOUT') {
...
client.on('auth_failure', (msg) => {
    ...
    // Clear session data on auth failure to force new login
    if (fs.existsSync(getWritableSessionDir())) {
        console.log('Clearing invalid session data...');
        fs.rmSync(getWritableSessionDir(), { recursive: true, force: true });
    }
});
...
process.on('unhandledRejection', (reason, promise) => {
    ...
    // Attempt to recover from unhandled rejections
    if (!isReady) {
        handleReconnect();
    }
});
...
const PORT = Number(process.env.BOT_PORT || process.env.PORT || 4002);
